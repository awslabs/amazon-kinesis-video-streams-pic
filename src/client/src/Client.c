...
STATUS checkIntermittentProducerCallback(UINT32 timerId, UINT64 currentTime, UINT64 customData)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        CHK(!ATOMIC_LOAD(&pKinesisVideoClient->base.shutdown), STATUS_CLIENT_SHUTTING_DOWN);
...
STATUS stopKinesisVideoStreamSync(STREAM_HANDLE streamHandle)
{
    ...
    pKinesisVideoClient->clientCallbacks.lockMutexFn(pKinesisVideoStream->pKinesisVideoClient->clientCallbacks.customData,
                                                     pKinesisVideoStream->pKinesisVideoClient->base.streamListLock);
    streamsListLock = TRUE;

    CHK_STATUS(stopStreamSync(pKinesisVideoStream));

    pKinesisVideoClient->clientCallbacks.unlockMutexFn(pKinesisVideoStream->pKinesisVideoClient->clientCallbacks.customData,
                                                       pKinesisVideoStream->pKinesisVideoClient->base.streamListLock);
    streamsListLock = FALSE;
...
STATUS kinesisVideoStreamGetStreamInfo(STREAM_HANDLE streamHandle, PPStreamInfo ppStreamInfo)
{
    ...
    CHK(pKinesisVideoStream != NULL && pKinesisVideoStream->pKinesisVideoClient != NULL && ppStreamInfo != NULL, STATUS_NULL_ARG);
...
STATUS kinesisVideoStreamParseFragmentAck(STREAM_HANDLE streamHandle, UPLOAD_HANDLE uploadHandle, PCHAR ackSegment, UINT32 ackSegmentSize)
{
    ...
    CHK(pKinesisVideoStream != NULL && pKinesisVideoStream->pKinesisVideoClient != NULL && ackSegment != NULL, STATUS_NULL_ARG);
    pKinesisVideoClient = pKinesisVideoStream->pKinesisVideoClient;
    ...
    // Lock the client streams list lock
    // Inside `parseFragmentAck` the TO_STREAM_HANDLE macro is used which access the streams list at an index
    pKinesisVideoClient->clientCallbacks.lockMutexFn(pKinesisVideoClient->clientCallbacks.customData, pKinesisVideoClient->base.streamListLock);
    streamListLocked = TRUE;
...
STATUS freeKinesisVideoClientInternal(PKinesisVideoClient pKinesisVideoClient, STATUS failStatus)
{
    ...
    ATOMIC_STORE(&pKinesisVideoClient->base.shutdown, TRUE);
    ...
    if (locked) {
        ...
        pKinesisVideoClient->base.streamListLock = INVALID_MUTEX_VALUE;
        locked = FALSE;
    }
    ...
    if (IS_VALID_MUTEX_VALUE(pKinesisVideoClient->base.lock)) {
        ...
        pKinesisVideoClient->base.lock = INVALID_MUTEX_VALUE;
    }
    ...
    if (IS_VALID_MUTEX_VALUE(pKinesisVideoClient->base.putFrameLock)) {
        ...
        pKinesisVideoClient->base.putFrameLock = INVALID_MUTEX_VALUE;
    }
