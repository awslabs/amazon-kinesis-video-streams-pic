...
STATUS flushProfileDataToFile() {
    ...
    CHK(gFileLogger->currentProfileOffset != 0, retStatus);

    if (gFileLogger->currentpFileIndex >= gFileLogger->maxFileCount) {
        fileIndexToRemove = gFileLogger->currentpFileIndex - gFileLogger->maxFileCount;
        filePathLen = SNPRINTF(filePath, ARRAY_SIZE(filePath), "%s%s%s.%" PRIu64, gFileLogger->logFileDir, FPATHSEPARATOR_STR,
                FILE_LOGGER_PROFILE_LOG_FILE_NAME, fileIndexToRemove);
    ...
    filePathLen = SNPRINTF(filePath, ARRAY_SIZE(filePath), "%s%s%s.%" PRIu64, gFileLogger->logFileDir, FPATHSEPARATOR_STR, FILE_LOGGER_PROFILE_LOG_FILE_NAME, gFileLogger->currentpFileIndex);
    CHK(filePathLen <= MAX_PATH_LEN, STATUS_PATH_TOO_LONG);
    ...
    charLenToWrite = MIN(gFileLogger->currentProfileOffset, gFileLogger->stringProfileBufferLen - 1);
    gFileLogger->profileStringBuffer[charLenToWrite] = '\0';
    CHK_STATUS(writeFile(filePath, TRUE, FALSE, (PBYTE) gFileLogger->profileStringBuffer, charLenToWrite * SIZEOF(CHAR)));
    gFileLogger->currentpFileIndex++;

    ULLTOSTR(gFileLogger->currentpFileIndex, fileIndexBuffer, ARRAY_SIZE(fileIndexBuffer), 10, &fileIndexStrSize);
    retStatus = writeFile(gFileLogger->indexpFilePath, TRUE, FALSE, (PBYTE) fileIndexBuffer, (STRLEN(fileIndexBuffer)) * SIZEOF(CHAR));
...
CleanUp:
    ...
    if (gFileLogger != NULL) {
        gFileLogger->currentProfileOffset = 0;
    }
...
//            va_start(valist, fmt);
//            offset = vsnprintf(gFileLogger->profileStringBuffer + gFileLogger->currentProfileOffset, gFileLogger->stringProfileBufferLen - gFileLogger->currentProfileOffset,
//                               logFmtString, valist);
//            va_end(valist);
//
...
//            if (offset > 0 && gFileLogger->currentProfileOffset + offset >= gFileLogger->stringProfileBufferLen) {
//                status = flushProfileDataToFile();
//                if (STATUS_FAILED(status)) {
//                    PRINTF("flush log to file failed with 0x%08x\n", status);
//                }
//                // even if flushProfileDataToFile failed, currentOffset will still be reset to 0
//                va_start(valist, fmt);
//                offset = vsnprintf(gFileLogger->profileStringBuffer + gFileLogger->currentProfileOffset,
//                                   gFileLogger->stringProfileBufferLen - gFileLogger->currentProfileOffset,
//                                   logFmtString, valist);
//                va_end(valist);
//
...
//                if (offset > gFileLogger->stringProfileBufferLen) {
//                    PRINTF("truncating log message as it can't fit into string buffer here\n");
//                    offset = (INT32)
//                    gFileLogger->stringProfileBufferLen - 1;
//                }
//            }
//
//            if (offset < 0) {
...
#endif
//        if(level == LOG_LEVEL_PROFILE) {
//            gFileLogger->currentProfileOffset += offset;
//        } else {
                        ...
                        logPrintFunc* pFilePrintFn)
{
    ...
    gFileLogger->stringProfileBufferLen = 0;
...
STATUS createFileLoggerWithProfiling(UINT64 maxStringBufferLen, UINT64 maxLogFileCount, PCHAR logFileDir, BOOL printLog, BOOL setGlobalLogFn,
                        BOOL enableProfileFileLogging, logPrintFunc* pFilePrintFn)
{
    STATUS retStatus = STATUS_SUCCESS;
    CHK(gFileLogger == NULL, retStatus); // dont allocate again if already allocated
    CHK(maxStringBufferLen <= MAX_FILE_LOGGER_STRING_BUFFER_SIZE && maxStringBufferLen >= MIN_FILE_LOGGER_STRING_BUFFER_SIZE &&
    ...
    BOOL fileFound = FALSE;
    CHAR fileIndexBuffer[KVS_COMMON_FILE_INDEX_BUFFER_SIZE];
    CHAR fileIndexpBuffer[KVS_COMMON_FILE_INDEX_BUFFER_SIZE];
    UINT64 charWritten = 0, indexFileSize = KVS_COMMON_FILE_INDEX_BUFFER_SIZE;

    // allocate the struct and string buffer together
    gFileLogger = (PFileLogger) MEMALLOC(SIZEOF(FileLogger) + maxStringBufferLen * SIZEOF(CHAR) + 5 * 1024 * SIZEOF(CHAR));
    MEMSET(gFileLogger, 0x00, SIZEOF(FileLogger));
    // point stringBuffer to the right place
    gFileLogger->stringBuffer = (PCHAR)(gFileLogger + 1);
    gFileLogger->stringBufferLen = maxStringBufferLen;
    gFileLogger->profileStringBuffer = (PCHAR)(gFileLogger + 1) + gFileLogger->stringBufferLen;
    gFileLogger->stringProfileBufferLen = 5 * 1024;
    gFileLogger->lock = MUTEX_CREATE(FALSE);
    gFileLogger->currentOffset = 0;
    gFileLogger->maxFileCount = maxLogFileCount;
    gFileLogger->currentFileIndex = 0;
    ...
    charWritten = SNPRINTF(gFileLogger->indexpFilePath, MAX_PATH_LEN + 1, "%s%s%s", gFileLogger->logFileDir, FPATHSEPARATOR_STR,
                           FILE_LOGGER_LAST_PROFILE_INDEX_FILE_NAME);
    CHK(charWritten <= MAX_PATH_LEN, STATUS_PATH_TOO_LONG);
    gFileLogger->indexpFilePath[charWritten] = '\0';

    indexFileSize = KVS_COMMON_FILE_INDEX_BUFFER_SIZE;
    CHK_STATUS(fileExists(gFileLogger->indexpFilePath, &fileFound));
    if (fileFound) {
        CHK_STATUS(readFile(gFileLogger->indexpFilePath, FALSE, NULL, &indexFileSize));
        CHK(indexFileSize < KVS_COMMON_FILE_INDEX_BUFFER_SIZE, STATUS_FILE_LOGGER_INDEX_FILE_INVALID_SIZE);
        CHK_STATUS(readFile(gFileLogger->indexpFilePath, FALSE, (PBYTE) fileIndexpBuffer, &indexFileSize));
        fileIndexpBuffer[indexFileSize] = '\0';
        STRTOUI64(fileIndexpBuffer, NULL, 10, &gFileLogger->currentpFileIndex);
    }
...
STATUS freeFileLogger()
{
    ...
    if (IS_VALID_MUTEX_VALUE(gFileLogger->lock)) {
        ...
        retStatus = flushProfileDataToFile();
        if (STATUS_FAILED(retStatus)) {
            PRINTF("flush log to file failed with 0x%08x\n", retStatus);
        }
